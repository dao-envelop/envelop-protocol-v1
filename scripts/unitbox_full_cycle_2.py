...
def main():
    ...
    wrapper.removeERC20Collateral(wnft721.address, wTokenId, dai2.address, {"from": accounts[1], "gas_price": price}) #by investor
    ...
    assert dai2.balanceOf(unitbox.address) == before_dai2_balance3 + coll_amount*wrapper.getWrappedToken(wnft721.address, wTokenId)[5][2][1]/10000
    ...
    before_dai2_balance3 = dai2.balanceOf(accounts[3])
    before_usdt_balance3 = usdt.balanceOf(accounts[3])
    unitbox.swapMeWithPath(dai2.address, [dai2.address, unitbox.dexForChain()[2], usdt.address], {"from": accounts[0], "gas_price": price})
