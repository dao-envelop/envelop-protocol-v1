...
###wrap, add Collateral, remove collateral, manual swap through direct swap pair, unwrap
...
def main():
    #cross swap
    #UniswapPairV2 niftsy20/WETH 0xf3a383ab34f084d6ef9ff375238816fc65153146
    ...
    niftsy20 = TokenMock.at('0xF0783F8628a2eE77Dae3e0D11245d9c1dD60Ebf7') #swapable token
    ...
    #whitelist.setWLItem((2, niftsy20.address), wl_data, {"from": accounts[0], "gas_price": price})
    ...
    #add collateral - niftsy20. niftsy20 has DEX
    ...
    #claim collateral niftsy20
    before_niftsy20_balance1 = niftsy20.balanceOf(accounts[1])
    before_niftsy20_balance2 = niftsy20.balanceOf(accounts[2])
    before_niftsy20_balance3 = niftsy20.balanceOf(accounts[3])
    before_usdt_balance3 = usdt.balanceOf(accounts[3])
    wrapper.removeERC20Collateral(wnft721.address, wTokenId, niftsy20.address, {"from": accounts[1], "gas_price": price}) #by investor

    assert niftsy20.balanceOf(accounts[1]) == before_niftsy20_balance1 + coll_amount*wrapper.getWrappedToken(wnft721.address, wTokenId)[5][0][1]/10000
    assert niftsy20.balanceOf(accounts[2]) == before_niftsy20_balance2 + coll_amount*wrapper.getWrappedToken(wnft721.address, wTokenId)[5][1][1]/10000
    assert niftsy20.balanceOf(unitbox.address) == before_niftsy20_balance3 + coll_amount*wrapper.getWrappedToken(wnft721.address, wTokenId)[5][2][1]/10000
    ...
    before_niftsy20_balance3 = niftsy20.balanceOf(accounts[3])
    before_usdt_balance3 = usdt.balanceOf(accounts[3])
    unitbox.swapMe(niftsy20.address, {"from": accounts[1], "gas_price": price})

    assert niftsy20.balanceOf(unitbox.address) == 0
    assert before_usdt_balance3 < usdt.balanceOf(accounts[3])

    unitbox.unWrap(wnft721.address, wTokenId, {"from": accounts[2]})
