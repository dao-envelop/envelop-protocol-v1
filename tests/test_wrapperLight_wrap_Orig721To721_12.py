...
def test_addColl(accounts, erc721mock, wrapperLight, dai, weth, wnft721ForWrapperLightV1, niftsy20, erc721mock1, erc1155mock1, wrapperCheckerLightV1):
    ...
    wTokenId1 = makeFromERC721ToERC721WithoutCollateralLight(accounts, erc721mock, wrapperLight, wnft721ForWrapperLightV1, niftsy20, ORIGINAL_NFT_IDs[1], accounts[3])
    ...
    erc721mock1.approve(wrapperLight.address, ORIGINAL_NFT_IDs[2], {"from": accounts[1]} )
    erc721mock1.approve(wrapperLight.address, ORIGINAL_NFT_IDs[3], {"from": accounts[1]} )
    ...
    dai.approve(wrapperLight.address, 2*amount, {"from": accounts[1]})
    weth.transfer(accounts[1], 7*amount, {"from": accounts[0]})
    weth.approve(wrapperLight.address, 7*amount, {"from": accounts[1]})

    #add collateral
    wrapperLight.addCollateral(wnft721ForWrapperLightV1.address, wTokenId1, [((3, erc721mock1.address), ORIGINAL_NFT_IDs[2], 0),
    ...
    #2
    collateral1 = wrapperLight.getWrappedToken(wnft721ForWrapperLightV1, wTokenId1)[1]
    assert wrapperLight.balance() == "1.1 ether"
    assert erc1155mock1.balanceOf(wrapperLight.address, ORIGINAL_NFT_IDs[2]) == coll_amount
    assert erc1155mock1.balanceOf(wrapperLight.address, ORIGINAL_NFT_IDs[3]) == coll_amount
    assert erc721mock1.ownerOf(ORIGINAL_NFT_IDs[2]) == wrapperLight.address
    assert erc721mock1.ownerOf(ORIGINAL_NFT_IDs[3]) == wrapperLight.address
    assert dai.balanceOf(wrapperLight.address) == 3*amount
    assert weth.balanceOf(wrapperLight.address) == 17*amount
    ...
    #check 
    assert wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, dai.address)[0] == 2*amount
    assert wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, weth.address)[0] == 7*amount
    assert wrapperCheckerLightV1.getNativeCollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1) == "0.1 ether"
    assert wrapperCheckerLightV1.getERC1155CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, erc1155mock1.address, ORIGINAL_NFT_IDs[2])[0] == coll_amount 



    contract_eth_balance = wrapperLight.balance()
    before_dai_balance = wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId, dai.address)[0]+wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, dai.address)[0]
    before_weth_balance = wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId, weth.address)[0]+wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, weth.address)[0]
    before_eth_balance = wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId, zero_address)[0]+wrapperCheckerLightV1.getERC20CollateralBalance(wnft721ForWrapperLightV1.address, wTokenId1, zero_address)[0]
    before_acc_balance = accounts[3].balance()

    wrapperLight.unWrap(3, wnft721ForWrapperLightV1.address, wTokenId, {"from": accounts[3]})
    wrapperLight.unWrap(3, wnft721ForWrapperLightV1.address, wTokenId1, {"from": accounts[3]})
