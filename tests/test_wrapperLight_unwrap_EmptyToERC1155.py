...
def test_unwrap(accounts, wrapperLight, dai, weth, wnft1155ForWrapperLightV1, niftsy20, erc1155mock1, erc721mock1):
    ...
    eth_contract_balance = wrapperLight.balance()
    ...
    assert erc1155mock1.balanceOf(wrapperLight.address, ORIGINAL_NFT_IDs[1]) == in_nft_amount
    assert wrapperLight.balance() == eth_amount
    assert wnft1155ForWrapperLightV1.balanceOf(accounts[3], wTokenId) == out_nft_amount
    ...
    wnft1155ForWrapperLightV1.safeTransferFrom(accounts[3], accounts[4], wTokenId, 1, "", {"from": accounts[3]} )

    with reverts("ERC115 unwrap available only for all totalSupply"):
        wrapperLight.unWrap(out_type, wnft1155ForWrapperLightV1.address, wTokenId, {"from": accounts[3]} )


    niftsy20.approve(wrapperLight.address, transfer_fee_amount, {"from": accounts[4]})
    niftsy20.transfer(accounts[4], transfer_fee_amount, {"from": accounts[0]})
    wnft1155ForWrapperLightV1.safeTransferFrom(accounts[4], accounts[3], wTokenId, 1, "", {"from": accounts[4]} )
