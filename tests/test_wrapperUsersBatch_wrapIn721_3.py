...
# repeat erc20 tokens in collateral info
def test_wrapBatch_ownerSBT(accounts, erc721mock, wrapperUsersBatch, dai, weth, wnft721SBT1forBatch, niftsy20):
	...
	dai.approve(wrapperUsersBatch.address, 2 * call_amount * count, {'from':accounts[0]})
	...
	tx = wrapperUsersBatch.wrapBatch(
		wNFTs, 
		[dai_data, eth_data, weth_data, dai_data, eth_data], 
		receivers, 
		wnft721SBT1forBatch,  
		{"from": accounts[0], "value": eth_amount * count})

	# new dai data
	dai_data = (dai_property, 0, 2 * Wei(call_amount))
	j = 0
	while j < count:
	...
	assert dai.balanceOf(wrapperUsersBatch) == 2 * call_amount * count
...
def test_addCollateralBatch_ownerSBT1(accounts, erc721mock, wrapperUsersBatch, dai, weth, wnft721SBT1forBatch, niftsy20):
	...
	#checks
	assert wrapperUsersBatch.balance() == 2 * eth_amount * count
	assert dai.balanceOf(wrapperUsersBatch) == 4 * call_amount * count
	assert weth.balanceOf(wrapperUsersBatch) == 4 * call_amount * count

	wNFT = wrapperUsersBatch.getWrappedToken(wnft721SBT1forBatch, count - 1)

	# new collateral data for one token
	dai_data = (dai_property, 0, 4 * Wei(call_amount))
	weth_data = (weth_property, 0, 4 * Wei(call_amount))
	eth_data = (eth_property, 0, 2 * eth_amount)
