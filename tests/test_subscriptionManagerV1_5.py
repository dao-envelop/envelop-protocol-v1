...
def test_settings(accounts, erc721mock, wrapperTrustedV1, dai, weth, wnft721ForwrapperTrustedV1, niftsy20, saftV1, whiteListsForTrustedWrapper, subscriptionManager):
    ...
    subscriptionType = (timelockPeriod, 0, 1, True)
    subscriptionManager.addTarif((subscriptionType, payOption, services), {"from": accounts[0]})
    ...
    dai.transfer(accounts[1], 2*payAmount, {"from": accounts[0]} )
    dai.approve(subscriptionManager.address, 2*payAmount, {"from": accounts[1]})
    ...
    tx = subscriptionManager.buySubscription(1,1, accounts[1], {"from": accounts[1]})
    assert wnft721ForwrapperTrustedV1.balanceOf(accounts[0]) == 1
    assert wnft721ForwrapperTrustedV1.balanceOf(accounts[1]) == 1
    #check tickets
    ...
    assert subscriptionManager.getUserTickets(accounts[1])[0][1] == 0
    assert subscriptionManager.getUserTickets(accounts[1])[0][0] == 0
    assert subscriptionManager.getUserTickets(accounts[1])[1][1] == 1
    assert subscriptionManager.getUserTickets(accounts[1])[1][0] > 0
...
def test_wrapBatch(accounts, erc721mock, wrapperTrustedV1, dai, weth, wrapper, wnft721ForwrapperTrustedV1, niftsy20, saftV1, whiteListsForTrustedWrapper, subscriptionManager):
    ...
    #accounts[1] wraps for accounts[0]
    ...
    assert subscriptionManager.getUserTickets(accounts[1])[0][1] == 0
    assert subscriptionManager.getUserTickets(accounts[1])[0][0] == 0
    assert subscriptionManager.getUserTickets(accounts[1])[1][1] == 0
    assert subscriptionManager.getUserTickets(accounts[1])[1][0] > 0
    
    #try to wrap again - accounts[1]
