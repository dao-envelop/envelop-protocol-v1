...
def test_addCollateral(accounts, erc721mock, wrapper, dai, weth, wnft721, niftsy20, erc1155mock1, erc721mock1, whiteLists):
    ...
    with reverts("Only trusted address"):
        wrapper.addCollateralUnsafe(wnft721.address, wTokenId, [], {"from": accounts[9], "value": eth_amount})

    wrapper.setTrustedAddres(accounts[1], True, {"from": accounts[0]})
    wrapper.addCollateralUnsafe(wnft721.address, wTokenId, [dai_data], {"from": accounts[1], "value": eth_amount})
